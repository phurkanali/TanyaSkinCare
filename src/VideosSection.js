  ...
  return (
    <section id="videos" className="py-16 bg-white">
      <div className="max-w-6xl mx-auto px-4">
        <h3 className="text-3xl font-semibold text-gray-900 mb-2">Most Viewed Videos</h3>
        <p className="text-sm text-gray-600 mb-6">Total Views: <strong>{formattedTotalViews}</strong></p>
          ...
          {videos.map((video) => (
            <div key={video.id} className="bg-pink-50 rounded-lg shadow hover:shadow-lg transition-shadow overflow-hidden">
              ...
              <div className="p-4">
                ...
                <a href={`https://www.youtube.com/watch?v=${video.id}`} target="_blank" rel="noreferrer" className="inline-block mt-3 px-4 py-2 bg-pink-500 text-white rounded-lg text-sm hover:bg-pink-600">Watch Now</a>
              </div>
            </div>
          ))}
        </div>
